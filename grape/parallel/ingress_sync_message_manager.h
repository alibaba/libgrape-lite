...
#ifndef GRAPE_PARALLEL_INGRESS_SYNC_MESSAGE_MANAGER_H_
#define GRAPE_PARALLEL_INGRESS_SYNC_MESSAGE_MANAGER_H_
...
/**
 * @brief IngressAsyc message manager.
 *
 * The send and recv methods are not thread-safe.
 */
class IngressAsycMessageManager : public MessageManagerBase {
 public:
  IngressAsycMessageManager() : comm_(NULL_COMM) {}
  ~IngressAsycMessageManager() override {
    if (ValidComm(comm_)) {
      MPI_Comm_free(&comm_);
    }
  }
   ...
   * vertex in F_1. This function invoked on F_1 send manager_status_ on b' to b on F_2,
  ...
  /**
   * 同时发送消息来源, 表示 s->v 发送了一个消息 msg
  */
  template <typename GRAPH_T, typename MESSAGE_T>
  inline void SyncStateOnOuterVertex(const GRAPH_T& frag,
                                     const typename GRAPH_T::vid_t& s,
                                     const typename GRAPH_T::vertex_t& v,
                                     const MESSAGE_T& msg) {
    fid_t fid = frag.GetFragId(v);
    to_send_[fid] << s << frag.GetOuterVertexGid(v) << msg;
  }
  ...
  */
  ...
  inline void SyncStateDepend(vid_t* value_depend, 
                                        vid_t* global_depend, 
                                        vid_t depend_num) {
    MPI_Allgather(value_depend, depend_num, MPI_INT,
        global_depend, depend_num, MPI_INT, comm_);
  }
  ...
  */
  template <typename GRAPH_T, typename MESSAGE_T>
  inline bool GetMessage(const GRAPH_T& frag, typename GRAPH_T::vid_t& s, typename GRAPH_T::vertex_t& v,
                         MESSAGE_T& msg) {
    while (cur_ != fnum_ && to_recv_[cur_].Empty()) {
      ++cur_;
    }
