#ifndef EXAMPLES_ANALYTICAL_APPS_PAGERANK_PAGERANK_PULL_H_
#define EXAMPLES_ANALYTICAL_APPS_PAGERANK_PAGERANK_PULL_H_
...
class PagerankPullContext : public grape::VoidContext<FRAG_T> {
 public:
  ...
  ~PagerankPullContext() {
    ...
    LOG(INFO) << "GPU " << dev << " Compute time: " << traversal_kernel_time * 1000
    << " ms Comm time: " << mm->GetAccumulatedCommTime() * 1000
    << " ms Ratio: " << traversal_kernel_time / mm->GetAccumulatedCommTime();
  }
#endif
