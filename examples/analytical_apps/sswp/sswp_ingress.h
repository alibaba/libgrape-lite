...
#ifndef ANALYTICAL_APPS_SSWP_SSWP_INGRESS_H_
#define ANALYTICAL_APPS_SSWP_SSWP_INGRESS_H_
...
class SSWPIngress : public TraversalAppBase<FRAG_T, VALUE_T> {
 public:
  ...
  delta_t GetInitDelta(const vertex_t& v) const override {
    ...
    if (native_source && source == v) {
      init_dist = std::numeric_limits<value_t>::max();
    }
  ...
  //   return lhs.SetIfGreaterThan(rhs);
  // }
  bool accumulate(delta_t& lhs, const delta_t& rhs) override {
    return atomic_max(lhs.value , rhs.value);
  } 
  ...
  void Compute(const vertex_t& u, const value_t& value, const delta_t& delta,
    ...
    if (FLAGS_cilk) {
      ...
      granular_for(j, 0, out_degree, (out_degree > 1024), {
        ...
        // auto new_dist = (dist < e.data ? dist : e.data);
...
#endif  // ANALYTICAL_APPS_SSWP_SSWP_INGRESS_H_
