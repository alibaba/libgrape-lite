#ifndef EXAMPLES_ANALYTICAL_APPS_SSSP_SSSP_BF_H_
#define EXAMPLES_ANALYTICAL_APPS_SSSP_SSSP_BF_H_
...
class SSSPBFContext : public grape::VoidContext<FRAG_T> {
 public:
...
#endif
  explicit SSSPBFContext(const FRAG_T& frag)
  ...
  bool accept() {
    ...
    if(ws_scheduler->local_id() == 0) {
      ...
      if(max_*1000 <= 30.0) to_process = true;
    }
...
template <typename FRAG_T>
class SSSPBF : public GPUAppBase<FRAG_T, SSSPBFContext<FRAG_T>>,
               public ParallelEngine {
 public:
  INSTALL_GPU_WORKER(SSSPBF<FRAG_T>, SSSPBFContext<FRAG_T>, FRAG_T)
  ...
  void PEval(const fragment_t& frag, context_t& ctx,
    ...
    ForEach(stream, ws_in,
            [=] __device__(vertex_t v) mutable { d_in_q.Append(v); });
...
#endif  // EXAMPLES_ANALYTICAL_APPS_SSSP_SSSP_BF_H_
