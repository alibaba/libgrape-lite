#ifndef EXAMPLES_ANALYTICAL_APPS_SSSP_SSSPSTAT_H_
#define EXAMPLES_ANALYTICAL_APPS_SSSP_SSSPSTAT_H_
#include "app_config.h"
#include "grape_gpu/grape_gpu.h"

namespace grape_gpu {
template <typename FRAG_T>
class SSSPStatContext : public grape::VoidContext<FRAG_T> {
 public:
  using vid_t = typename FRAG_T::vid_t;
  using oid_t = typename FRAG_T::oid_t;
  using vertex_t = typename FRAG_T::vertex_t;
  using dist_t = uint32_t;
  explicit SSSPStatContext(const FRAG_T& frag)
  ...
  void Output(std::ostream& os) override {
    ...
    for (auto v : iv) {
      os << frag.GetId(v) << " " << visited_count[v] << std::endl;
    }
  }
  ...
  //////////////STATITICS
  VertexArray<uint32_t, vid_t> visited_count;
};

// A SSSP without priority scheduling for statistic purposes
template <typename FRAG_T>
class SSSPStat : public GPUAppBase<FRAG_T, SSSPStatContext<FRAG_T>>,
                 public ParallelEngine {
 public:
  INSTALL_GPU_WORKER(SSSPStat<FRAG_T>, SSSPStatContext<FRAG_T>, FRAG_T)
  ...
  void IncEval(const fragment_t& frag, context_t& ctx,
    ...
    auto& visited_count = ctx.visited_count;
    auto d_visited_count = visited_count.DeviceObject();
    ...
    ForEach(stream, WorkSourceArray<vertex_t>(in_q.data(), in_size),
            [=] __device__(vertex_t v) mutable {
              atomicAdd(&d_visited_count[v], 1);
            });
...
#endif  // EXAMPLES_ANALYTICAL_APPS_SSSP_SSSPSTAT_H_
