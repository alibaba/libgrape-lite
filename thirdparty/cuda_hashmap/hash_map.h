...
*/	}
        template<class key_t, class value_t, class hash_t>
        __device__ HashMapNode<key_t,value_t>* HashMap<key_t,value_t,hash_t>::find(const key_t & key){
          uint32_t index=hash(key);
	...
	template<class key_t, class value_t, class hash_t>
	__host__ HashMap<key_t,value_t,hash_t> * CreateHashMap(cudaStream_t cuda_stream, uint32_t num_buckets, uint32_t num_elements){
