...
int main(int argc, char *argv[]) {
	...
	DefineMapV(init) {
		for_nb(if(nb_id>v_id)e.push_back(make_pair(nb_w,make_pair(v_id,nb_id))))
	};

	vertexMap(All, CTrueV, init);

	All.Block(kruskal(e,mst0.data(),n_vertex); Reduce(mst0,mst,vector<E> e; e.assign(mst0,mst0+len); e.insert(e.end(),mst,mst+len); kruskal(e,mst,len+1)));
