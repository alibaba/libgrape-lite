...
#ifndef GRAPE_GPU_FRAGMENT_BASIC_FRAGMENT_LOADER_H_
#define GRAPE_GPU_FRAGMENT_BASIC_FRAGMENT_LOADER_H_
...
inline bool operator<(const grape::internal::Vertex<OID_T, VDATA_T>& lhs,
                      const grape::internal::Vertex<OID_T, VDATA_T>& rhs) {
...
inline bool operator<(const grape::Edge<VID_T, EDATA_T>& lhs,
                      const grape::Edge<VID_T, EDATA_T>& rhs) {
  if (lhs.src() != rhs.src()) {
...
class BasicFragmentLoader<
    ...
    typename std::enable_if<!std::is_same<typename FRAG_T::vdata_t,
                                          grape::EmptyType>::value>::type> {
 ...
 public:
  explicit BasicFragmentLoader(const grape::CommSpec& comm_spec)
  ...
  bool SerializeFragment(std::shared_ptr<fragment_t>& fragment,
    ...
    snprintf(serial_file, sizeof(serial_file), "%s/%s",
             serialization_prefix.c_str(),
             grape::kSerializationVertexMapFilename);
 ...
 private:
  ...
  void vertexRecvRoutine() {
    grape::ShuffleInPair<oid_t, vdata_t> data_in(comm_spec_.fnum() - 1);
  ...
  void edgeRecvRoutine() {
    grape::ShuffleInTriple<oid_t, oid_t, edata_t> data_in(comm_spec_.fnum() -
                                                          1);
 ...
 private:
  ...
  std::vector<grape::ShuffleOutPair<oid_t, vdata_t>> vertices_to_frag_;
  std::vector<grape::ShuffleOutTriple<oid_t, oid_t, edata_t>> edges_to_frag_;
...
class BasicFragmentLoader<
 ...
 public:
  ...
  void AddVertex(const oid_t& id, const grape::EmptyType& data) {}
  ...
  void ConstructFragment(std::shared_ptr<fragment_t>& fragment) {
    ...
    if (rebalance_) {
      ...
      grape::Rebalancer<fragment_t> rb(comm_spec_, rebalance_vertex_factor_);
    ...
    fragment = std::shared_ptr<fragment_t>(new fragment_t(vm_ptr_));
    std::vector<grape::internal::Vertex<vid_t, grape::EmptyType>> fake_vertices;
 ...
 private:
      ...
      for (int tid = 0; tid < thread_num; ++tid) {
        process_threads[tid] = std::thread([&]() {
          ...
          while (true) {
            ...
            grape::Edge<vid_t, edata_t>* ptr = &processed_edges_[wu.begin];
            ...
            while (src_iter != src_end) {
              ...
              vm_ptr_->GetGid(u_fid, *src_iter, u_gid);
              vm_ptr_->GetGid(v_fid, *dst_iter, v_gid);
              ptr->SetEndpoint(u_gid, v_gid);
  ...
  void constructVMThreadRoutine(
      fid_t fid,
      grape::BlockingQueue<std::tuple<std::vector<oid_t>, std::vector<oid_t>,
  ...
  void initMirrorInfo(std::shared_ptr<fragment_t> fragment) {
    ...
    std::thread recv_thread([&]() {
      ...
      for (int i = 1; i < worker_num; ++i) {
        ...
        MPI_Recv(&offsets[0], sizeof(vid_t) * 2, MPI_CHAR, src_worker_id, 0,
                 comm_spec_.comm(), MPI_STATUS_IGNORE);
        grape_gpu::VertexRange<vid_t> range(offsets[0], offsets[1]);
        gid_list.clear();
        gid_list.resize(range.size());
        MPI_Recv(&gid_list[0], gid_list.size() * sizeof(vid_t), MPI_CHAR,
                 src_worker_id, 0, comm_spec_.comm(), MPI_STATUS_IGNORE);
...
#endif  // GRAPE_GPU_FRAGMENT_BASIC_FRAGMENT_LOADER_H_
