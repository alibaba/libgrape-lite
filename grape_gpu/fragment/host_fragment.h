...
#ifndef GRAPE_GPU_FRAGMENT_HOST_FRAGMENT_H_
#define GRAPE_GPU_FRAGMENT_HOST_FRAGMENT_H_
...
class HostFragment {
 public:
  ...
  std::shared_ptr<coo_t> ConvertToCOO(COOIdType id_type,
                                      bool release_csr = false) {
    if (coo_frag_ == nullptr) {
      if (release_csr) {
        ReleaseDeviceCSR();
      }
      coo_frag_ = std::make_shared<coo_t>(vm_ptr_, id_type);
      coo_frag_->Init(*this);
    }
    return coo_frag_;
  }
 ...
 protected:
  ...
  template <typename _OID_T, typename _VID_T, typename _VDATA_T, typename _EDATA_T>
  friend class COOFragment;
};
}  // namespace grape_gpu
#endif  // GRAPE_GPU_FRAGMENT_HOST_FRAGMENT_H_
