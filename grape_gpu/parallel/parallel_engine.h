#ifndef GRAPE_GPU_PARALLEL_PARALLEL_ENGINE_H_
#define GRAPE_GPU_PARALLEL_PARALLEL_ENGINE_H_
...
LoadBalancing ParseLoadBalancing(const std::string& s_lb) {
  if (s_lb == "auto_static" || s_lb == "AUTO_STATIC") {
    return LoadBalancing::kAutoStatic;
  } else if (s_lb == "auto" || s_lb == "AUTO") {
    return LoadBalancing::kAuto;
  } else if (s_lb == "cmold" || s_lb == "CMOLD") {
...
DEV_INLINE void LBCMOld(const FRAG_T& dev_frag,
  ...
  auto* vertices =
      (VertexMetadata<vid_t,
                      metadata_t>*) &shared_lb_cm_old[0];  // len = block_size
  auto** nbr_begin =
...
DEV_INLINE void LBWARP(const FRAG_T& dev_frag, const WORK_SOURCE_T& work_source,
  ...
  auto* vertices =
      (VertexMetadata<vid_t,
                      metadata_t>*) &shared_lb_warp[0];  // len = block_size
  auto** nbr_begin =
      (const nbr_t**) &vertices[blockDim.x];  // len == block size
  auto* row_offset =
...
DEV_INLINE void LBCTA(const FRAG_T& dev_frag, const WORK_SOURCE_T& work_source,
  ...
  for (auto i = 0 + tid; i < size_rup; i += nthreads) {
    ...
    dev::CTAWorkScheduler<vid_t, edata_t, vertex_metadata_t,
...
class ParallelEngine {
 public:
  ...
  void Init(const FRAG_T& frag) {
    degree_.resize(frag.GetVerticesNum());
    prefix_sum_.resize(frag.GetVerticesNum());
  }
  ...
  inline void ForEachEdgeStrict(const Stream& stream, const FRAG_T& dev_frag,
    ...
    size_t partition_size = round_up(total_edges, block_num);
...
#endif  // GRAPE_GPU_PARALLEL_PARALLEL_ENGINE_H_
