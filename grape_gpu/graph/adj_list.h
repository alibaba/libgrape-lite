...
#ifndef GRAPE_GPU_GRAPH_DEV_ADJ_LIST_H_
#define GRAPE_GPU_GRAPH_DEV_ADJ_LIST_H_
...
struct Nbr {
  ...
  void GetEdgeSrc(const grape::Edge<VID_T, EDATA_T>& edge) {
    neighbor.SetValue(edge.src());
    data = edge.edata();
  }

  void GetEdgeDst(const grape::Edge<VID_T, EDATA_T>& edge) {
...
struct Nbr<VID_T, grape::EmptyType> {
  ...
  DEV_HOST Nbr(const VID_T& nbr_, const grape::EmptyType&) : neighbor(nbr_) {}
  DEV_HOST Nbr(const Vertex<VID_T>& nbr_, const grape::EmptyType&)
      : neighbor(nbr_) {}
  DEV_HOST ~Nbr() {}
  ...
  void GetEdgeSrc(const grape::Edge<VID_T, grape::EmptyType>& edge) {
    neighbor.SetValue(edge.src());
  }

  void GetEdgeDst(const grape::Edge<VID_T, grape::EmptyType>& edge) {
    neighbor.SetValue(edge.dst());
  }

  DEV_HOST_INLINE Vertex<VID_T> get_neighbor() const { return neighbor; }
  DEV_HOST_INLINE grape::EmptyType get_data() const { return data; }
 ...
 public:
  ...
  class iterator {
   ...
   public:
    ...
    DEV_HOST_INLINE reference_type operator*() const noexcept {
      return *current_;
    }
    DEV_HOST_INLINE pointer_type operator->() const noexcept {
      return current_;
    }
    ...
    DEV_HOST_INLINE bool operator==(const iterator& rhs) noexcept {
  ...
  class const_iterator {
   ...
   public:
    ...
    DEV_HOST_INLINE const_iterator& operator++() noexcept {
      ++current_;
      return *this;
    }

    DEV_HOST_INLINE const_iterator operator++(int) noexcept {
    ...
    DEV_HOST_INLINE const_iterator operator+(size_t offset) noexcept {
      return const_iterator(current_ + offset);
    }

    DEV_HOST_INLINE bool operator==(const const_iterator& rhs) noexcept {
      return current_ == rhs.current_;
    }
  ...
  DEV_HOST_INLINE iterator begin() { return iterator(begin_); }
  DEV_HOST_INLINE iterator end() { return iterator(end_); }

  DEV_HOST_INLINE const_iterator begin() const {
    return const_iterator(begin_);
  }
  DEV_HOST_INLINE const_iterator end() const { return const_iterator(end_); }

  DEV_HOST_INLINE NbrT* begin_pointer() { return begin_; }
  DEV_HOST_INLINE const NbrT* begin_pointer() const { return begin_; }

  DEV_HOST_INLINE NbrT* end_pointer() { return end_; }
  DEV_HOST_INLINE const NbrT* end_pointer() const { return end_; }

  DEV_HOST_INLINE bool empty() const { return end_ == begin_; }
 ...
 * all neighbors in format of Nbr, which contains the other Node and the data
 on
 * the Edge.
 *
 * @tparam VID_T
...
template <typename VID_T>
inline grape::InArchive& operator<<(grape::InArchive& archive,
                                    const Nbr<VID_T, grape::EmptyType>& nbr) {
...
#endif  // GRAPE_GPU_GRAPH_ADJ_LIST_H_
